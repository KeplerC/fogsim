...
class PathManager:
    ...
    def get_target_waypoint(self, current_pos, sync_mode=False):
        ...
        # More reasonable completion threshold: within 5 meters of final waypoint when near end
        completion_threshold = max(5.0, lookahead_m * 1.5)
        if near_end and final_dist < completion_threshold:
            self.completed = True
...
class CollisionHandler(BaseHandler):
    ...
    def launch(self):
        ...
        self.client.set_timeout(60.0)  # Increased timeout for Docker containers
        
        # Initialize Traffic Manager with custom port to avoid bind conflicts
        if 'tm_port' in self.config['simulation']:
            self.traffic_manager = self.client.get_trafficmanager(self.config['simulation']['tm_port'])
        
        # Load world with error handling
        try:
            print(f"Loading Town03 world on port {self.config['simulation']['port']}...")
            self.world = self.client.load_world("Town03")
            print("Successfully loaded Town03")
        except carla.client.TimeoutException as e:
            print(f"Timeout while loading world: {e}")
            raise
    ...
    def _collision_callback(self, event):
        ...
        if not self.has_collided:  # Only record first collision
            self.has_collided = True
            self.collision_tick = self.tick
            print(f"Collision detected at tick {self.tick}!")
    ...
    def close(self):
        ...
        # Cleanup sensors with error handling
        if self.collision_sensor:
            try:
                self.collision_sensor.stop()
                self.collision_sensor.destroy()
            except Exception as e:
                print(f"Error destroying collision sensor: {e}")
        ...
        for camera_name, camera in self.cameras.items():
            if camera:
                try:
                    ...
                    print(f"Error destroying camera {camera_name}: {e}")
        ...
        # Cleanup vehicles with error handling
        if self.ego_vehicle:
            try:
        ...
        if self.obstacle_vehicle:
            try:
                if self.obstacle_vehicle.is_alive:
                    self.obstacle_vehicle.destroy()
            except Exception as e:
                print(f"Error destroying obstacle vehicle: {e}")
        
        # Restore world settings
        if self.world and hasattr(self, 'original_settings') and self.original_settings:
            try:
    ...
    def step_with_action(self, action):
        ...
        # Check if all waypoints are finished for both vehicles
        ego_finished = self.ego_path_manager is None or self.ego_path_manager.is_completed()
        obstacle_finished = self.obstacle_path_manager is None or self.obstacle_path_manager.is_completed()
        all_waypoints_finished = ego_finished and obstacle_finished
        
        # Check termination conditions: (1) all waypoints finished OR (2) tick > 949
        termination = all_waypoints_finished or self.tick > 949
        timeout = self.tick >= (self.config['ego_vehicle']['go_straight_ticks'] +
                               self.config['ego_vehicle']['turn_ticks'] +
                               self.config['ego_vehicle']['after_turn_ticks'])
        success = not self.has_collided and termination  # Success if completed without collision
        
        info = {
            'tick': self.tick,
            'has_collided': self.has_collided,
            'ego_finished': ego_finished,
            'obstacle_finished': obstacle_finished,
            'all_waypoints_finished': all_waypoints_finished
        }
...
def plot_dual_vehicle_trajectories(ego_ref, ego_actual, obs_ref, obs_actual, output_path=None, show_arrows=True, sync_mode=False, scenario_type=None, run_number=None):
    ...
    plt.legend(loc='upper center', bbox_to_anchor=(0.5, 1.5), ncol=6)
    plt.grid(True, alpha=0.3)
...
def run_first_simulation(config, ego_trajectory_file=None, obstacle_trajectory_file=None):
    ...
    client.set_timeout(60.0)  # Increased timeout for Docker containers
    
    # Initialize Traffic Manager with custom port to avoid bind conflicts
    if 'tm_port' in config['simulation']:
        print(f"Initializing Traffic Manager on port {config['simulation']['tm_port']}")
        traffic_manager = client.get_trafficmanager(config['simulation']['tm_port'])
        # Small delay to ensure TM is fully initialized
        import time
        time.sleep(1)
...
#     camera = world.spawn_actor(camera_bp, camera_transform, attach_to=None)
        ...
        for step in range(total_steps):
            ...
            # Stop simulation immediately upon collision
            if handler.has_collided and not has_collided:
                has_collided = True
                print(f"Collision detected at tick {handler.tick}! Stopping simulation.")
                break
            
            # Check termination conditions from step_with_action
            if termination:
                print(f"Simulation terminated at step {step}. Reason: {'All waypoints finished' if info.get('all_waypoints_finished', False) else 'Tick limit exceeded (>949)'}")
                break
    ...
    # Get collision tick from handler if collision occurred
    collision_tick = handler.collision_tick if 'handler' in locals() and handler.collision_tick is not None else None
    return has_collided, current_delta_k, collision_tick
...
def run_monte_carlo_simulation(config, num_samples=10, output_dir='./results', use_fogsim=False, no_risk_eval=False):
    ...
    for sample in range(num_samples):
        ...
        try:
            ...
            has_collided, current_delta_k, collision_tick = run_adaptive_simulation_fogsim(
...
def main():
    ...
    # Update Traffic Manager port
    base_config['simulation']['tm_port'] = args.tm_port
        ...
        try:
            ...
            with open(stats_file, 'w') as f:
                ...
                for case in stats['collision_cases']:
                    ...
                    if 'collision_tick' in case and case['collision_tick'] is not None:
                        f.write(f"  Collision tick: {case['collision_tick']}\n")
                    if 'error' in case:
