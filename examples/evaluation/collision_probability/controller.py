...
class VehiclePIDController:
    ...
    def __init__(self, vehicle, args_lateral, args_longitudinal, offset=0, max_throttle=0.75, max_brake=0.3,
        ...
        """
        Constructor method.
        :param args_lateral: dictionary of arguments for lateral PID controller
        :param args_longitudinal: dictionary of arguments for longitudinal PID controller  
        :param offset: offset from center line
        """
    ...
    def run_step(self, cur_speed, target_speed, cur, wp, is_reverse=False):
        """
        Execute one step of control invoking both lateral and longitudinal PID controllers
        :param cur_speed: current vehicle speed
        :param target_speed: desired vehicle speed
        :param cur: current vehicle state (x, y, angle)
        :param wp: target waypoint (x, y, angle)
        :param is_reverse: whether to reverse
...
class PIDLongitudinalController:
    """
    PIDLongitudinalController implements longitudinal control using PID.
    """
    ...
    def run_step(self, current_speed, target_speed, debug=False):
        """
        Execute one step of longitudinal control to reach target speed.
...
class PIDLateralController:
    """
    PIDLateralController implements lateral control using PID.
    """
    
    def __init__(self, offset=0, K_P=1.0, K_I=0.0, K_D=0.0, dt=0.03):
        """
        Constructor method.
        :param offset: distance to center line
        :param K_P: Proportional term
        :param K_D: Differential term
        :param K_I: Integral term  
        :param dt: time differential in seconds
    ...
    def run_step(self, cur, wp, is_reverse=False):
        """
        Execute one step of lateral control to steer towards waypoint.
        :param cur: current vehicle state (x, y, angle)
        :param wp: target waypoint (x, y, angle)
        :param is_reverse: whether reversing
        :return: steering control in range [-1, 1]
        """
    ...
    def _pid_control(self, cur, wp, is_reverse):
        ...
        # Heading error
        heading_error = desired_heading - cur.angle
        ...
        cross_track_error = np.sin(heading_error) * distance
        
        # Store error for PID
        self._e_buffer.append(cross_track_error)
        ...
        # PID output
        steering = (self._k_p * cross_track_error + 
                   self._k_d * de + 
                   self._k_i * ie + 
                   0.1 * heading_error)  # Add heading correction
